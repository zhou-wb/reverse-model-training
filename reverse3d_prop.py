...
class Reverse3dProp(nn.Module):
    def __init__(self) -> None:
        ...
        self.reverse_cnn = UnetGenerator(input_nc= 3, output_nc=1,
    ...
    def forward(self, input):
        ...
        input = utils.pad_image(input, target_shape=(512, 768), pytorch=True, stacked_complex=False)
        input = utils.crop_image(input, target_shape=(512, 768), pytorch=True, stacked_complex=False)
        
        slm_phase = self.reverse_cnn(input)      # input.shape = torch.Size([1, 8, 1080, 1920])
        
        slm_phase = utils.pad_image(slm_phase, target_shape=(480,640), pytorch=True, stacked_complex=False)
        slm_phase = utils.crop_image(slm_phase, target_shape=(480,640), pytorch=True, stacked_complex=False)
